...
const suppressions = exec(
  `SHOW VIEWS LIKE '%_ALERT_SUPPRESSION' IN SCHEMA ${rules_schema}`
)

const select_all_suppressions_sql = suppressions
  .map(
    (s) => `
  SELECT id, '$${s.name}' rule, TRUE suppressed FROM ${rules_schema}.$${s.name}
  UNION ALL
...
const suppressions_updates =
  select_all_suppressions_sql.length == 0
...
const NEW_SUPPRESSIONS_TBL = `${results_schema}.ALERT_SUPPRESSIONS_$${run_id}`
...
const QUERIES_LIKE_SQL = QUERIES_LIKE
  ? `  AND dst.alert:QUERY_NAME ILIKE '$${QUERIES_LIKE}'`
  : ''
