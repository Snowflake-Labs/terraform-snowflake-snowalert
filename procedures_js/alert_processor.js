...
WHERE alert:ACTOR = ?
  ...
  AND event_time >  
      CASE REGEXP_SUBSTR(LOWER(COALESCE(alert:CORRELATION_PERIOD, ''$${CORRELATION_PERIOD_MINUTES}'')), '[a-z]')
        WHEN 's' THEN DATEADD(seconds, - TO_NUMBER(REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, ''$${CORRELATION_PERIOD_MINUTES}''), '\\d+')), ?)
        WHEN 'm' THEN DATEADD(minutes, - TO_NUMBER(REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, ''$${CORRELATION_PERIOD_MINUTES}''), '\\d+')), ?)
        WHEN 'h' THEN DATEADD(hours, - TO_NUMBER(REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, ''$${CORRELATION_PERIOD_MINUTES}''), '\\d+')), ?)
        WHEN 'd' THEN DATEADD(days, - TO_NUMBER(REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, ''$${CORRELATION_PERIOD_MINUTES}''), '\\d+')), ?)
      END
