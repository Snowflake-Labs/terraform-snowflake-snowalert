...
WHERE alert:ACTOR = ?
  ...
  AND event_time >  
      DATEADD(
        CASE 
            WHEN REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, $${CORRELATION_PERIOD_MINUTES}), '[a-zA-Z]+') = 's' THEN 'SECONDS'
            WHEN REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, $${CORRELATION_PERIOD_MINUTES}), '[a-zA-Z]+') = 'm' THEN 'MINUTES'
            WHEN REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, $${CORRELATION_PERIOD_MINUTES}), '[a-zA-Z]+') = 'h' THEN 'HOURS'
            WHEN REGEXP_SUBSTR(COALESCE(alert:CORRELATION_PERIOD, $${CORRELATION_PERIOD_MINUTES}), '[a-zA-Z]+') = 'd' THEN 'DAYS'
        END,
