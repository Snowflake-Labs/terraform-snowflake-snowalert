...
# $$
# ;
# resource "snowflake_function" "urlencode_results" {
#   name     = "urlencode"
#   database = snowflake_database.snowalert.name
#   schema   = snowflake_schema.results.name
...
resource "snowflake_function" "slack_handler" {
  ...
  return_type = "VARIANT"
  statement   = <<SQL
  ${snowflake_database.snowalert.name}.${snowflake_schema.results.name}.${snowflake_function.slack_snowflake_chat_post_message.name}(
  payload['channel'],
  payload['message']
)
SQL
...
locals {
  ...
  geff_api_integration_name   = module.geff_snowalert.api_integration_name
  geff_api_gateway_invoke_url = module.geff_snowalert.api_gateway_invoke_url
