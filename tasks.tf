...
resource "snowflake_task" "snowalert_alert_merge_task" {
  ...
  sql_statement = "CALL ${local.snowalert_database_name}.${local.results_schema_name}.${snowflake_procedure.alert_merge.name}('30m')"
  enabled       = true
...
resource "snowflake_task" "snowalert_suppression_merge_task" {
  ...
  sql_statement = "CALL ${local.snowalert_database_name}.${local.results_schema_name}.${snowflake_procedure.alert_suppressions_runner_without_queries_like.name}()"
  enabled       = true
